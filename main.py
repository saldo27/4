...
class CalendarViewScreen(Screen):
    ...
    def export_summary_pdf(self, month_stats):
        ...
        try:
            ...
            # Create file name with full path for PDF
            import os
            month_name = self.current_date.strftime('%B_%Y')
            filename = os.path.abspath(f"schedule_summary_{month_name}.pdf")
        
            # Log the file path for debugging
            logging.info(f"Attempting to create PDF at: {filename}")
            ...
            # Add title
            title = Paragraph(f"Schedule Summary - {month_name}", styles['Heading1'])
            ...
            date_info = Paragraph(f"Generated on: {current_date}", styles['Normal'])
            user_info = Paragraph(f"Generated by: {app.current_user}", styles['Normal'])
            ...
            # Show error for missing ReportLab
