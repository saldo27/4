...
class CalendarViewScreen(Screen):
    ...
    def export_summary_pdf(self, month_stats):
        ...
        try:
            # Import ReportLab components first to catch import errors early
            try:
                from reportlab.lib.pagesizes import letter
                from reportlab.platypus import SimpleDocTemplate, Paragraph, Spacer, Table, TableStyle
                from reportlab.lib.styles import getSampleStyleSheet
                from reportlab.lib import colors
                logging.info("Successfully imported ReportLab components")
            except ImportError as imp_err:
                logging.error(f"Failed to import ReportLab: {str(imp_err)}")
                popup = Popup(
                    title='Error',
            ...
            filename = os.path.join(os.getcwd(), f"schedule_summary_{month_name}.pdf")
            logging.info(f"PDF file will be saved at: {filename}")
            ...
            # Add title
            elements.append(Paragraph(f"Schedule Summary - {month_name}", styles['Heading1']))
            ...
            elements.append(Paragraph(f"Generated by: {app.current_user}", styles['Normal']))
            ...
            try:
                ...
                elements.append(Spacer(1, 20))
                logging.info("Added statistics table")
            except Exception as table_err:
                logging.error(f"Failed to create statistics table: {str(table_err)}")
        
            # Add worker details
            elements.append(Paragraph("Worker Details", styles['Heading2']))
            elements.append(Spacer(1, 12))
            ...
            for worker_id, stats in sorted(month_stats['workers'].items()):
                try:
                    # Worker header
                    elements.append(Paragraph(f"Worker {worker_id}", styles['Heading3']))
                    ...
                    worker_table = Table(worker_data, colWidths=[120, 80])
                    ...
                    # Shifts listing
                    elements.append(Paragraph("Assigned Shifts:", styles['Normal']))
                    worker_shifts = month_stats['worker_shifts'].get(worker_id, [])
                    ...
                    if worker_shifts:
                        ...
                        for shift in sorted(worker_shifts, key=lambda x: x['date']):
                            ...
                            shift_data.append([
                                ...
                                shift['day'],
                                f"Post {shift['post']}",
                                "WEEKEND" if shift['is_weekend'] else (
                                    "HOLIDAY" if shift['is_holiday'] else "Regular"
                                )
                            ])
                    
                        shift_table = Table(shift_data, colWidths=[80, 80, 60, 80])
                    ...
                    logging.info(f"Added data for worker {worker_id}")
                except Exception as worker_err:
                    logging.error(f"Error adding worker {worker_id} data: {str(worker_err)}")
        
            # Build the PDF with extensive error trapping
            try:
                logging.info("Building PDF document...")
                doc.build(elements)
                logging.info(f"PDF successfully built at {filename}")
                ...
                logging.error(f"Failed to build PDF: {str(build_err)}")
                raise  # Re-raise to be caught by outer exception handler
            
        except Exception as outer_err:
            # Show more informative error popup with details
            error_message = f"Failed to export PDF: {str(outer_err)}"
